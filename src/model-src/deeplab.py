...
num_of_classes = 4 # define the number of classes in the dataset
...
parser.add_argument("-nn", "--nn_model", help="select model path for inference", default='models/4_class_model_mobilenet_v3_large_data4_combined_class_weights_512x512_without_softmax.blob', type=str)
...
with dai.Device() as device:
    ...
    while True:
        ...
        # print("lay1 shape", lay1.shape)
        found_classes = np.unique(lay1)
        legend = ""
        legendmap = {0: "Soil (Black)", 1: "Clover (Red)", 2: "Broadleaf (Purple)", 3: "Grass (Orange)"}
        ...
        cv2.putText(frame, "Found classes {}".format(legend), (2, 10), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (255, 0, 0))
        cv2.imshow("nn_input", frame)
        # cv2.imshow("segmask", output_colors)
        counter+=1
